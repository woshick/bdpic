...
define(function (require) {
	...
	var Layer = Backbone.View.extend({
		...
		modelId: 0,
		events: {
			'swipeUp .img-container img': 'showUpImage',
		...
		render: function (id) {
			...
			// 如果model是空的化，说明list没有初始化(先路由进大图页，列表页没有初始化collection)，或者是collection没有该id对应的model
			// 此时我们要将他渲染到list页面
			if (!model) {
				// 将该页面跳转到list
				location.hash = '#'
			...
			// 将当前图片id缓存下来
			this.lastModelId.push(this.modelId)//写在这里可防止刷新产生的id
			// 获取json数据
			var data = model.pick('url', 'title')
			data.styles = 'line-height: ' + height + 'px';
			// 渲染模板
